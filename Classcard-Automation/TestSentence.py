...
def _plan_wrong(total):
    """0~1개만 일부러 틀리게 (90점 패스 기준 안전). 틀릴 문항 순번(1-based) 집합."""
    if not total or total <= 0:
        return set()
    n_wrong = random.randint(0, 1)
    if n_wrong == 0:
        return set()
...
def run_automation_loop(driver, answer_dict, stop_event: threading.Event):
    ...
    flipped_qids = set()    # SPACE로 flip 시도한 문제 (중복 방지)
    ...
    try:
        while not stop_event.is_set():
            ...
            # 앞면(한글) → SPACE로 flip
            if not flipped:
                if qid not in flipped_qids:
                    _press_space(driver)
                    flipped_qids.add(qid)
